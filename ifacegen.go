...
const (
  {{range $i, $m := $x.Methods}}call_{{$x.Struct}}_{{$m.Method}} = {{$i}}
  {{end}}
)
...
{{range $x.Methods}}
func (m {{$x.Receiver}}) {{.FuncSig}} {
  atomic.AddInt32(&m.callCounts[call_{{$x.Struct}}_{{.Method}}], 1)
...
func (m {{$x.Receiver}}) {{.Method}}CallCount() int {
  return int(atomic.LoadInt32(&m.callCounts[call_{{$x.Struct}}_{{.Method}}]))
}
{{end}}
{{end}}
`))
